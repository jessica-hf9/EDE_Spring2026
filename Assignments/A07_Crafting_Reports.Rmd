...
summary_PM25 <- epa_data %>% 
  drop_na(PM2.5) %>% 
  group_by(COUNTY) %>% 
  summarize(mean_PM2.5 = mean(PM2.5), .groups = "drop") %>% 
  arrange(mean_PM2.5) %>% 
...
summary_Ozone <- epa_data %>% 
  drop_na(Ozone) %>% 
  group_by(COUNTY) %>% 
  summarize(mean_Ozone = mean(Ozone), .groups = "drop") %>% 
  arrange(mean_Ozone) %>% 
  mutate(mean_Ozone = round(mean_Ozone, 2)) %>% 
  rename(County = COUNTY)

#Output tables (set column name with MathJax)
...
```{r ozone.boxplot.2018, echo=FALSE, fig.align="left", fig.height=3, fig.cap="Ozone distribution by month (2018)."}
epa_data %>% 
...
```{r ozone.boxplot.2019, echo=FALSE, fig.align="left", fig.height=3, fig.cap="Ozone distribution by month (2019)."}
epa_data %>% 
...
## Git Repository
https://github.com/jessica-hf9/EDE_Spring2026 

I did not use AI in the completion of this assignment. 

Juanying Huangfu, 2026-03-02