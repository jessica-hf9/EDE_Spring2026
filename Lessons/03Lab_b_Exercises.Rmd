...
n_sites <- n_distinct(pm25$Site.ID)
n_sites

# Compute the mean and median PM 2.5 concentration
mean_pm25 <- mean(pm25$Daily.Mean.PM2.5.Concentration, na.rm = TRUE)
median_pm25 <- median(pm25$Daily.Mean.PM2.5.Concentration, na.rm = TRUE)
...
min_date <- min(pm25$Date, na.rm = TRUE)
max_date <- max(pm25$Date, na.rm = TRUE)
max_date
min_date
```
...
ggplot(pm25, aes(x = Source)) +
  geom_bar() +
  labs(x = "Source", y = "Number of samples")
