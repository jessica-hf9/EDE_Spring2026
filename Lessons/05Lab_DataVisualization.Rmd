...
class(month(PeterPaul.chem.nutrients$sampledate))

#List unique values
sort(unique(month(PeterPaul.chem.nutrients$sampledate)))
...
PeterPaul.chem.nutrients %>% 
  ...
  ggplot(aes(x=DOY, y=temperature_C, color=year4)) +
  geom_point(alpha = 0.7, size = 0.8) +
  facet_wrap(~ lakename, nrow = 2) +
  labs(x = "Day of year", y = "Surface temperature (°C)", color = "Year")
...
# Change x axis to include marker/labels every 5 years
  ...
  labs(x = "Date", y = "Temperature (°C)", color = "Depth (m)")
...
EPA_wide <- EPAair %>%
  select(Date, site, county, AQSparameter, AQI, latitude, longitude) %>%
  group_by(Date, site, county, AQSparameter) %>%
  summarize(
